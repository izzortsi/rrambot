...
g_secret_key = os.environ.get("API_SECRET")
...
user_data = sub_client.subscribe_user_data_event(
    listen_key, callback, error_handler=error
)
user_data
...
def ccandles_callback(data_type: "SubscribeMessageType", event: "any"):
    print(data_type)
    print(event)
...
sub_client.subscribe_continuous_candlestick_event(
    "ethusdt", "perpetual", "1m", ccandles_callback
)
