...
class Looker:
    ...
    def look(self, trades = None):
        ...
        subhover = HoverTool(
            tooltips=[
                ( 'time',   '@date{%m/%d %H:%M}'            ),
                ( 'histogram',  '@{histogram}{%0.2f}' ), # use @{ } for field names with spaces
            ],
            ...
            # display a tooltip whenever the cursor is vertically in line with a glyph
            mode='vline', line_policy='nearest', names=['histogram']
        )
        ...
        fig = figure(x_axis_type="datetime", title=f'{self.symbol}, {self.tframe}, {self.fromdate}',
                    ...
                    x_range=(self.df.close.index[range_tool_a], self.df.close.index[range_tool_b]),
                    y_range = cylims
                    )
        #hylims = (min(self.df.histogram[self.df.histogram.notna()])*0.95, (max(self.df.histogram[self.df.histogram.notna()])*1.05))

        subfig = figure(x_axis_type="datetime", title="MACD histogram",
                    plot_width=950, plot_height=150,
        ...
        hist = subfig.line('date', 'histogram', source= source, color='red', name='histogram')
        ...
        if trades != None:
            for i, trade in enumerate(trades[:-1]):
                ...
                entry_price = self.df.loc[self.df.index == entry_date]["close"].values[0]
                ...
                if exit_price > entry_price:
                    poly = PolyAnnotation(
                        ...
                        ys=[entry_price - abs(exit_price - entry_price)*0.1, entry_price + abs(exit_price - entry_price)*0.1, exit_price + abs(exit_price - entry_price)*0.1, exit_price - abs(exit_price - entry_price)*0.1],
                    )
                    fig.add_layout(poly)
                else:
                    poly = PolyAnnotation(
