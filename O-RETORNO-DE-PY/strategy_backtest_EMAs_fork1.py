...
df.ema25.iloc[:25]
##
df.ema25.iloc[:25].head(5)
##
np.alltrue(df.ema25 > df.ema50)
##
df.close.iloc[5:]
##
for (i, price) in enumerate(df.close.iloc[5:]):
    print(i, price)
##
indicators = df.loc[:, df.columns != "close"]
period = 25
ema25 = indicators["ema25"]
ema50 = indicators["ema50"]
ema100 = indicators["ema100"]
...
#emas alignment
cond1 = False
if (np.alltrue(ema25.iloc[:400].head(period) > ema50.iloc[:400].head(period)) 
and np.alltrue(ema50.iloc[:400].head(period) > ema100.iloc[:400].head(period))):
    cond1 = True
print(cond1)
##
np.alltrue(ema25.iloc[150:400].head(period) > ema50.iloc[150:400].head(period))
##
ema25.iloc[150:400].head(period)
ema50.iloc[150:400].head(period)
